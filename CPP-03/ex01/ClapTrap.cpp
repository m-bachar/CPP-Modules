...
void	ClapTrap::takeDamage(unsigned int amount)
{
	...
	hitPoints -= amount;
	if (hitPoints > 0)
		std::cout << "ClapTrap " << PURPLE << name << RESET << " has taken " << RED << amount << " damage points !" << RESET << std::endl;
	else
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (hitPoints > 0 && energyPoints > 0)
	{
		hitPoints += amount;
		energyPoints--;
		std::cout << "ClapTrap " << PURPLE << name << RESET << " has taken a painkiller and increased his health by " << GREEN << amount << " hit points !" << RESET << std::endl;
	}
	else
